...
for cls in base_exp_classes:
    ...
    for idx, training_batch in enumerate(updated_ordering):
        ...
        for t_cls in training_batch:
            ...
            tr_predictions, n_tr_features, tr_features = model.test(step_x)
            ...
            mh_distances, _ = replay_manager.get_mhdistance(val_features.numpy(), t_labels)
            mean_distances, _ = replay_manager.get_mean_distance(val_features.numpy(), t_labels)
            n_mh_distances, _ = replay_manager.get_mhdistance(n_val_features.numpy(), t_labels)
            n_mean_distances, _ = replay_manager.get_mean_distance(n_val_features.numpy(), t_labels)

            labels.extend(t_labels)
            mh_distance.extend(mh_distances)
            euc_distance.extend(mean_distances)
            n_mh_distance.extend(n_mh_distances)
            n_euc_distance.extend(n_mean_distances)
        ...
        metric_manager.gen_box_plots(n_mh_distance, labels, 'NORM_val_box_baseclass-{}_epoch-{}.png'.format(cls, idx))
        metric_manager.gen_box_plots(n_euc_distance, labels, 'NORMval_box_euclidean_baseclass-{}_epoch-{}.png'.format(cls, idx))
